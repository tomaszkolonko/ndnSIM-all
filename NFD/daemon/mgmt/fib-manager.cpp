                       ...
                       ControlResponse& response)
{
  ...
  if(debug) {
	  std::cout << "INSIDE FibManager::addNextHop(ControlParameters&, ControlResponse&)" << std::endl;
	  std::cout << parameters.hasFaceId() << "   " << parameters.hasMac() << std::endl;
  ...
  if(debug) {
	  ...
	  std::cout << "add-nexthop prefix: " << prefix << " faceid: " << faceId << "cost: " << cost << " " << macAddress<< std::endl << std::endl;
  }
