...
FibUpdater::sendAddNextHopUpdate(const FibUpdate& update,
                                 ...
                                 const FibUpdateFailureCallback& onFailure,
                                 uint32_t nTimeouts)
{std::cout<< "edw mpainei? "<<update.mac<<std::endl;
...
FibUpdater::addInheritedRoutes(const RibEntry& entry, const Rib::Rib::RouteSet& routesToAdd)
{
  for (const Route& route : routesToAdd) {
    ...
    if (!entry.hasFaceId(route.faceId)) {
      ...
      addFibUpdate(FibUpdate::createAddUpdate(entry.getName(), route.faceId, route.cost, route.mac));
    }
  }
}
                               ...
                               const Route& ignore)
{
  for (const Route& route : routesToAdd) {
    if (route.faceId != ignore.faceId) {
      ...
      addFibUpdate(FibUpdate::createAddUpdate(name, route.faceId, route.cost, route.mac));
    }
  }
}
                                            ...
                                            const Route& existingRoute)
{
  ...
  if (costDidChange) {
    ...
    if (route.cost <= entry.getRouteWithLowestCostByFaceId(route.faceId)->cost) {
      ...
      addFibUpdate(FibUpdate::createAddUpdate(entry.getName(), prevRoute->faceId, prevRoute->cost, prevRoute->mac));
    }
                                               ...
                                               const bool captureWasTurnedOff)
{
  ...
  if (!entry.hasCapture() && entry.getNRoutes() != 0) {
    ...
    if (it != ancestorRoutes.end()) {
      ...
      addFibUpdate(FibUpdate::createAddUpdate(entry.getName(), it->faceId, it->cost, it->mac));
    }
  }
}
                            ...
                            Rib::Rib::RouteSet routesToRemove)
{
  ...
  for (Rib::RouteSet::const_iterator addIt = routesToAdd.begin(); addIt != routesToAdd.end(); ) {
    ...
    if (!entry.hasFaceId(addIt->faceId)) {
      ...
      addFibUpdate(FibUpdate::createAddUpdate(entry.getName(), addIt->faceId, addIt->cost, addIt->mac));
    }
