...
Route::wireEncode(EncodingImpl<TAG>& block) const
{
  ...
  totalLength += prependStringBlock(block,ndn::tlv::nfd::Mac,m_mac);
...
Route::wireDecode(const Block& wire)
{
  ...
  if (val != m_wire.elements_end() && val->type() == tlv::nfd::Mac) {
      m_mac = readNonNegativeInteger(*val);
      ++val;
    }
    else {
      BOOST_THROW_EXCEPTION(Error("Missing required Mac field"));
    }
